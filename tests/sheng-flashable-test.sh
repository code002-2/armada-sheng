...
# selection picks the vendor-ABL geometry.
set -euo pipefail
...
# root-partition binding (dual 'linux' / single 'userdata') is honored.
grep -q -- '--root-partition' "$ROOT/post_process/export-fastboot-images.sh" \
    || fail "export script lost --root-partition"
grep -q 'PARTLABEL_ROOT="${ROOT_PARTITION}"' "$ROOT/post_process/export-fastboot-images.sh" \
    || fail "export script does not apply --root-partition"
grep -q 'detect_partition' "$ROOT/post_process/export-fastboot-images.sh" \
    || fail "flash.sh lost partition auto-detection"
grep -q 'ROOT_PART}.*linux.img' "$ROOT/post_process/export-fastboot-images.sh" \
    || fail "flash.sh does not target the detected root partition"

# fstab binds by UUID and the layout is SINGLE-partition: /boot merged into
# root, no /boot entry, no linux_boot.img / no fastboot flash of it.
grep -q 'UUID=${ROOT_UUID} / ext4' "$ROOT/post_process/export-fastboot-images.sh" \
    || fail "fstab template lost UUID root binding (ext4)"
grep -q 'no /boot entry' "$ROOT/post_process/export-fastboot-images.sh" \
    || fail "fstab no longer documents the merged /boot"
if grep -q 'flash ${PARTLABEL_BOOT}\|linux_boot.img' "$ROOT/post_process/export-fastboot-images.sh"; then
    fail "single-partition export still references a separate /boot image"
fi
