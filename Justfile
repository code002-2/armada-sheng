...
[group('Build Sheng (Xiaomi Pad 6S Pro)')]
build-sheng-flashable $kernel_pkg $tag=default_tag:
    #!/usr/bin/env bash
    set -euxo pipefail
    # Sequential here (not dependency-declaration): the multi-dependency && /
    # expression form trips this just version's parser.
    just build-sheng-image "{{ kernel_pkg }}" "armada" "{{ tag }}"
    just _build-bib "armada" "{{ tag }}" raw disk_config/disk-sheng.toml
    just export-fastboot-images sm8550-xiaomi-sheng output/image/disk.raw output/fastboot
